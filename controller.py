...
class Controller:
    ...
    def update_fingers_status():
        ...
        Controller.index_finger_within_Thumb_finger = (Controller.hand_Landmarks.landmark[8].y > Controller.hand_Landmarks.landmark[4].y
                                                        and Controller.hand_Landmarks.landmark[8].y < Controller.hand_Landmarks.landmark[2].y)
        Controller.middle_finger_within_Thumb_finger = (Controller.hand_Landmarks.landmark[12].y > Controller.hand_Landmarks.landmark[4].y
                                                         and Controller.hand_Landmarks.landmark[12].y < Controller.hand_Landmarks.landmark[2].y)
        Controller.little_finger_within_Thumb_finger = (Controller.hand_Landmarks.landmark[20].y > Controller.hand_Landmarks.landmark[4].y
                                                         and Controller.hand_Landmarks.landmark[20].y < Controller.hand_Landmarks.landmark[2].y)
        Controller.ring_finger_within_Thumb_finger = (Controller.hand_Landmarks.landmark[16].y > Controller.hand_Landmarks.landmark[4].y
                                                       and Controller.hand_Landmarks.landmark[16].y < Controller.hand_Landmarks.landmark[2].y)
    ...
    def get_position(hand_x_position, hand_y_position):
        ...
        threshold = 5
        if current_x < threshold: current_x = threshold
        elif current_x > Controller.screen_width - threshold: current_x = Controller.screen_width - threshold
        if current_y < threshold: current_y = threshold
        elif current_y > Controller.screen_height - threshold: current_y = Controller.screen_height - threshold
        return (current_x, current_y)

    def cursor_moving():
        point = 9
        current_x = Controller.hand_Landmarks.landmark[point].x
        current_y = Controller.hand_Landmarks.landmark[point].y
    ...
    def detect_scrolling():
        scrolling_up = (Controller.little_finger_up and Controller.index_finger_down
                        and Controller.middle_finger_down and Controller.ring_finger_down)
        if scrolling_up:
            pyautogui.scroll(120)

        scrolling_down = (Controller.index_finger_up and Controller.middle_finger_down
                          and Controller.ring_finger_down and Controller.little_finger_down)
        if scrolling_down:
            pyautogui.scroll(-120)

    def detect_zoomming():
        zoomming = (Controller.index_finger_up and Controller.middle_finger_up
                    and Controller.ring_finger_down and Controller.little_finger_down)
        window = .05
    ...
    def detect_screenshot():
        ...
        if screenshot_condition and not Controller.screenshot_taken:
    ...
    def detect_volume():
        now = time.time()
        if now - Controller.last_vol_time < 0.3:
            return
        vol_gesture = (
            Controller.index_finger_up and
            Controller.Thump_finger_up and
            Controller.middle_finger_down and
            Controller.ring_finger_down and
            Controller.little_finger_down
        )
        if vol_gesture:
            thumb_y = Controller.hand_Landmarks.landmark[4].y
            if thumb_y < 0.4:
                ...
                print("Volume Down")
            Controller.last_vol_time = now

    def detect_brightness():
        now = time.time()
        if now - Controller.last_bright_time < 0.2:
            return
        bright_gesture = (
            Controller.index_finger_up and
        ...
        if bright_gesture:
            index_y = Controller.hand_Landmarks.landmark[8].y
            new_bright = int((1.0 - index_y) * 100)
            new_bright = max(0, min(100, new_bright))
            sbc.set_brightness(new_bright)
            Controller.last_bright_time = now
            print(f"Brightness: {new_bright}%")

    def get_gesture_label():
        if Controller.all_fingers_down:
            return "Dragging"
        if Controller.all_fingers_up and Controller.Thump_finger_down:
            return "Cursor Frozen"
        if Controller.index_finger_within_Thumb_finger and Controller.middle_finger_up:
            return "Left Click"
        if Controller.middle_finger_within_Thumb_finger and Controller.index_finger_up:
            return "Right Click"
        if Controller.ring_finger_within_Thumb_finger and Controller.index_finger_up:
            return "Double Click"
        if (Controller.little_finger_up and Controller.index_finger_down and
                Controller.middle_finger_down and Controller.ring_finger_down and
                Controller.Thump_finger_down):
            return "Screenshot"
        if (Controller.index_finger_up and Controller.Thump_finger_up and
                Controller.middle_finger_down):
            return "Volume Control"
        if (Controller.index_finger_up and Controller.middle_finger_up and
                Controller.ring_finger_down and Controller.little_finger_down and
                Controller.Thump_finger_down):
            return "Brightness / Zoom"
        if Controller.little_finger_up and Controller.index_finger_down:
            return "Scroll Up"
        if Controller.index_finger_up and Controller.little_finger_down:
            return "Scroll Down"
        return "Moving"